...
const ProfileScreen = ({navigation}) => {
  ...
  const [ogrTelefon,setOgrTelefon] = useState("");
    ...
    try {
      await AsyncStorage.getItem('UserData').then(value => {
        if (value !== null) {
          ...
          setOgrAd(user.name)
          setOgrMail(user.Email)
          setOgrTelefon(user.Tel)
        }
      });
    } catch (error) {
      console.log(error);
  ...
  return (
    <SafeAreaView style={{ backgroundColor: "whitesmoke", height: "100%" }}>
      ...
      <View style={{ justifyContent: "center", alignItems: "center", backgroundColor: "white", height: 180, borderBottomWidth: 0.5 }}>
        <View style={{ backgroundColor: "#93CAED", borderRadius: 50, height: 100, width: 100, justifyContent: "center", alignItems: "center" }}>
        <Avatar.Text
        size={90}
        label={ogrAd
          .split(' ')
      ...
      <ScrollView>
        ...
        <TouchableOpacity onPress={removeData} style={{ flexDirection: "row", height: "100%", alignItems: "center" }}>
